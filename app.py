...
def tambah_mahasiswa():
    ...
    if nim and nama and prodi:
        # Validasi apakah NIM sudah ada sebelumnya
        nim_int = int(nim)
        exists = any(mhs['nim'] == nim_int for mhs in MAHASISWA)
        if exists:
            flash("Gagal Tambah! NIM sudah digunakan mahasiswa lain.", "error")
        else:
            MAHASISWA.append({"nim": nim_int, "nama": nama, "prodi": prodi})
...
def hapus_mahasiswa(nim):
    ...
    mhs_awal = len(MAHASISWA)
    MAHASISWA = [mhs for mhs in MAHASISWA if mhs['nim'] != nim]
    
    if len(MAHASISWA) < mhs_awal:
        flash("Data Mahasiswa Berhasil Dihapus!", "success")
    else:
        flash("Gagal menghapus! Data tidak ditemukan.", "error")
